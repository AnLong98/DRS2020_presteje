...
class SnakePartManager:
    ...
    def generate_snake_for_player(self, player, table_width, table_height, snake_size, all_snakes, all_food, super_food):
        ...
        while True:
            ...
            is_colliding = self.collision_manager.is_coordinate_colliding(all_snakes, all_food, super_food, table_width, table_height, new_part)
            if not is_colliding:
                elements_stack.append(new_part)
                snake_size -= 1
                if self.generate_body_elements_for_snake(table_width, table_height, snake_size,
                                                          all_snakes, all_food, super_food, elements_stack):
                    return self.create_snake_from_drawable_components(elements_stack, player)



    def generate_body_elements_for_snake(self, table_width, table_height, elements_to_generate, all_snakes, all_food, super_food, elements_stack):
        ...
        for side in sides_to_check:
            if self.generate_drawable_component_on_tile(elements_stack, side[0] * self.part_width, side[1] * self.part_height,
                                                        all_snakes, all_food, super_food, table_width, table_height):
    ...
    def generate_drawable_component_on_tile(self, elements_stack, x_delta, y_delta, all_snakes, all_food, super_food, table_width, table_height):
        ...
        is_colliding = self.collision_manager.is_coordinate_colliding(all_snakes, all_food, super_food, table_width,
                                                                        table_height, part)
